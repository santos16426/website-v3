...
const SnapSection:React.FC<SnapSectionProps> = ({ sectionId, scrollTo, goTo, content,showScrollTo }) => {
  ...
  return (
    <div className='h-screen w-full relative snap-center' id={sectionId}>
      ...
      {goTo && scrollTo &&
      <div className={cn('absolute left-2/4 transform translate-x-[-50%] text-center bottom-[10px] cursor-pointer', !showScrollTo ? 'transition-opacity duration-500 opacity-0' : 'transition-opacity duration-500 opacity-100')} onClick={()=>scrollTo(goTo)}>
